    ...
    try {
      ...
      if (this.options.verbose && newSize > 0) {
        console.log(`📄 New file size: ${this.formatBytes(newSize)}`);
           ...
           const diffStr = sizeDiff > 0 ? `+${this.formatBytes(Math.abs(sizeDiff))}` : `-${this.formatBytes(Math.abs(sizeDiff))}`;
      ...
      if (stat.size !== newSize && newSize > 0) {
        console.warn(`⚠️  Warning: Downloaded file size (${this.formatBytes(stat.size)}) doesn't match expected size (${this.formatBytes(newSize)})`);
      }
