...
const WORKFLOWS: WorkflowConfig[] = [
  {
    ...
    event: "push",
    description:
      "Continuous Integration (format, lint, type check, test, build)",
  },
  {
    name: "security",
    file: "security.yml",
...
class GitHubActionsTestRunner {
  ...
  async runWorkflowTest(workflow: WorkflowConfig): Promise<TestResult> {
    ...
    try {
      ...
      console.log(
        `❌ ${workflow.name} workflow failed with error: ${errorMessage}\n`,
      );
