  ...
  describe('Flow Chart Diagrams', () => {
    it('should render flow chart diagram correctly', async () => {
      ...
      const flowChartSvg = await page.$eval('.mermaid-render-container svg', (svg) => {
        ...
        const nodes = svg.querySelectorAll('.node, .nodeLabel, [class*="node"], g[id*="flowchart"]');
        const edges = svg.querySelectorAll('.edgePath, path[class*="edge"], .path, .flowchart-link');
        return {
  ...
  describe('State Diagrams', () => {
    it('should render state diagram with transitions', async () => {
      ...
      const stateDiagramData = await page.evaluate(() => {
        ...
        return {
          hasStates: stateDiagram.querySelectorAll('.node, .state-group, [class*="state"], g[id*="state"]').length > 0,
          stateCount: stateDiagram.querySelectorAll('.node, .state-group, [class*="state"], g[id*="state"]').length,
          hasTransitions: stateDiagram.querySelectorAll('.edgePath, path[class*="edge"], .transition').length > 0,
          transitionCount: stateDiagram.querySelectorAll('.edgePath, path[class*="edge"], .transition').length
        };
      });
    ...
    it('should render git graph with commits and branches', async () => {
      ...
      const gitGraphData = await page.evaluate(() => {
        ...
        return {
          ...
          branchLabels: Array.from(gitGraph.querySelectorAll('text')).map(t => t.textContent).filter(t => t)
        };
      });
  ...
  describe('Entity Relationship Diagrams', () => {
    it('should render ER diagram with entities and relationships', async () => {
      ...
      const erDiagramData = await page.evaluate(() => {
        ...
        return {
          hasEntities: erDiagram.querySelectorAll('.er.entityBox, .entity, [class*="entity"], g[class*="er"]').length > 0,
          entityCount: erDiagram.querySelectorAll('.er.entityBox, .entity, [class*="entity"], g[class*="er"]').length,
          hasRelationships: erDiagram.querySelectorAll('.er.relationshipLine, .relation, [class*="relationship"], path[class*="er"]').length > 0,
          relationshipCount: erDiagram.querySelectorAll('.er.relationshipLine, .relation, [class*="relationship"], path[class*="er"]').length
        };
      });
  ...
  describe('Caching Behavior', () => {
    it('should cache rendered diagrams in sessionStorage', async () => {
      ...
      const cacheData = await page.evaluate(() => {
        const cacheKeys = Object.keys(sessionStorage).filter(key => key.startsWith('mermaid-cache-'));
        return {
    ...
    it('should use cached SVG on page reload', async () => {
      ...
      const cachedDiagrams = await page.$$eval('.mermaid-render-container svg', els => els.length);
    ...
    it('should handle invalid diagram syntax gracefully', async () => {
      ...
      const errorHandled = await page.evaluate(() => {
        ...
        return Array.from(containers).some(container => {
          return container.textContent?.includes('error') || 
